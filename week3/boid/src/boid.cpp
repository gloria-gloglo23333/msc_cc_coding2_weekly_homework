...
Boid::Boid()
{
	...
	velocity = ofVec3f(ofRandom(-0.5, 0.5), ofRandom(-0.5, 0.5));
}
...
ofVec3f Boid::separationGroup(std::vector<Boid *> &otherGroup) {
	ofVec3f average(0, 0, 0);
	for (int i = 0; i < otherGroup.size(); i++)
	{
		if (position.distance(otherGroup[i]->getPosition()) < separationGroupThreshold)
		{
			ofVec3f v = position - otherGroup[i]->getPosition();
			v.normalize();
			return v;
		}
	}
...
void Boid::update(std::vector<Boid *> &otherBoids, std::vector<Boid *> &otherGroup, ofVec3f &min, ofVec3f &max)
{
	...
	//velocity += separation(otherGroup);
